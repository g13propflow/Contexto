...
| `lead_created` | — |
| `lead_assigned` | — |
| `lead_entered_hitl` | — (se dispara al pasar `control_status` de `AUTOMATED` a `MANUAL`) |
...
> **Estado de cableado (punto de disparo real):** todos los triggers están cableados
> **excepto `visit_no_answer`**, que no tiene una transición de estado en el código
> (hoy solo existe `no_answer` de llamadas, no de visitas) — pendiente de crear ese
> estado antes de poder dispararlo. Puntos de disparo:
> `visit_scheduled` → `qualification_tools.schedule_visit` (nuevo) + endpoint
> `tour_scheduled`; `visit_confirmed` → `qualification_tools.confirm_visit`;
> `visit_no_show` → activity `mark_visit_no_show_activity`;
> `task_completed`/`task_cancelled` → `api/v1/tasks.py` (`update_task` y
> `complete_task`). Disparo genérico vía `app/services/workflow_trigger_dispatch.py`.
